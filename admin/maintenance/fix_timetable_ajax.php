...
try {
    ...
    // Get all assignments
    $assignments_query = "SELECT a.*, s.subject_code FROM assignments a JOIN subjects s ON a.subject_id = s.id";
    ...
    while ($assignment = $assignments_result->fetch_assoc()) {
        ...
        if (!is_array($preferred_days) || empty($preferred_days)) {
            $error_count++;
            $errors[] = "Assignment ID {$assignment_id} for subject {$assignment['subject_code']} has invalid days format";
            continue;
        }
    ...
    echo json_encode([
        ...
        'errors' => $errors,
        'message' => "Timetable fixed: $updated_count assignments updated with timetable entries."
    ]);
    ...
    error_log("Timetable fix error: " . $e->getMessage());
}
?> 